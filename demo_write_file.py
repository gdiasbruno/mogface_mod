...
if __name__ == "__main__":
    ...
    # For test single image
        ...
        with torch.no_grad():
            all_file = open("dataset/FDDB/FDDB-folds/FDDB-fold-all.txt", "r")
            lines = all_file.readlines()
            file = open("test_FINETUNED_30042023.txt", "w")

            for line in lines:
              line = line.replace("\n", "")
              img = cv2.imread("dataset/FDDB/originalPics/" + line + ".jpg")
              img_show = img.copy()
              t1 = time.time()
              boxes = process_img(img, net, generate_anchors_fn, normalize_setting)
              t2 = time.time()
              ...
              for i in range(len(boxes)):
                  file.write("{} {} {} {} {}\n".format(boxes[i][0], boxes[i][1], boxes[i][2], boxes[i][3], boxes[i][4]))
